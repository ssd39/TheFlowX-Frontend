...
elem.innerHTML = "";
elem.innerHTML += "<button class='btn' id='save'><img height='32px' width='32px' style='margin-right:5px;' src='imgs/save.png'>Save to IPFS</button><button class='btn' id='load'><img height='32px' width='32px' style='margin-right:5px;' src='imgs/import.png'>Load from IPFS</button><!--<button class='btn' id='download'><img height='32px' width='32px' style='margin-right:5px;' src='imgs/folder.png'>Download</button>-->";
...
elem.querySelector("#save").addEventListener("click",async function(){
	$("#loader").css('display','block');
	const data = JSON.stringify( graph.serialize() );
	const blob = new Blob( [ data ] );
	const cid = await upload_flow(new File([blob], 'flow.json'))
	$("#cid_text").html(`Your Flow file: <a href='https://cloudflare-ipfs.com/ipfs/${cid}/flow.json'>https://cloudflare-ipfs.com/ipfs/${cid}/flow.json</a>`)
	$("#cid_popup").css("display","block")
	console.log(cid)
	console.log("saved");
...
elem.querySelector("#load").addEventListener("click",function(){
	...
	console.log("loaded");*/
	//bafybeicionm7gacyhanyygnw7d4zbw7xlw4zd77vpbgwmuu765xxtfghyq
	$("#ldcid_popup").css("display","block")
});
